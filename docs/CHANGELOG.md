...
### Fixed
- `templates/dashboard.html`: 地図の市区町村塗りつぶしが全国618/4372局（約14%）で行われない不具合を修正
  - 原因: 観測点addr→市区町村名の変換 `extractCity` が、政令市の区（`浜松中央区高丘東`→GeoJSONは`浜松市`）や県名プレフィックス（`静岡森町森`→`森町`）を解決できず、塗り照合キーが一致しなかった
  - `extractCity` を廃止し `resolveCity`（政令市の区マッピング・最長前方一致・県名プレフィックス除去・異体字正規化）へ置換。照合先はその県のGeoJSONが実際に持つ `N03_004`/`N03_005` 集合
  - 政令市は区単位で塗り分け（浜松市の中央区/浜名区/天竜区などを各々の最大震度で塗る）。照合キーを区を持つ政令市は `pref::市::区`、それ以外は `pref::市町村` に統一
  - 全4372局のオフライン照合テスト（`src/test_city_matching.py`）で塗り対象の塗り漏れ0を確認（残13件はすべて空港＝塗り対象外）

### Changed
- `jma_intensity_web.py`: ダッシュボードHTML（`/`）にキャッシュ無効ヘッダ（`Cache-Control: no-cache, no-store, must-revalidate` 等）を付与
  - CSS/JSがHTMLにインライン埋め込みのため、ブラウザがHTMLをキャッシュするとデプロイ後も古い画面（古いJS）が表示される問題への対処
  - `/api/geojson`（不変な市区町村ポリゴン）にはキャッシュ制限を付けず、ブラウザキャッシュを維持
