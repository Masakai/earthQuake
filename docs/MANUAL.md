...
| `--confirm-window` | `10.0` | トリガ確認待ち時間 [秒]（I 値が確定してから発報するまでの最大待機時間） |
...
VoiceVox の音声合成に伴う遅延（数秒）が地震速報としては致命的なため、現在は **macOS `say -v Kyoko`** を固定エンジンとして使用しています。VoiceVox Engine が起動していても使用しません。
...
トリガ発火直後は計算窓に静穏期間のデータが残っているため、I 値が低めになります。そのため、トリガ発火から最大 `--confirm-window` 秒（既定 10 秒）待機し、I 値が安定した時点でアラートを発報してトリガ履歴に記録します。
