...
| ファイル | 説明 |
|---------|------|
| `jma_intensity_tui.py` | メイン TUI ダッシュボード |
| `jma_intensity_realtime.py` | JMA 計測震度コアライブラリ（TUI が import） |
| `simulate_udp.py` | 任意震度の合成波形を UDP 送出するシミュレーター |
| `verify_filter.py` | JMA フィルタ特性の検証スクリプト |
| `data/R38DC.xml` | StationXML（R38DC 感度情報） |
| `src/analyze_rs.py` | P2P地震情報・波形解析スクリプト（FDSN波形取得・スペクトログラム・白地図） |
...
```bash
.venv/bin/python3 jma_intensity_tui.py --station R38DC
```
...
ターミナル 1 でTUI を起動：
```bash
.venv/bin/python3 jma_intensity_tui.py --station R38DC --rt-window 5 --lta 10 --bind 127.0.0.1:9999
```

ターミナル 2 でシミュレーターを起動：
```bash
.venv/bin/python3 simulate_udp.py --intensity 3.0 --duration 60 --quiet-sec 20 --dest 127.0.0.1:9999
```
