...
1. [インストール](#1-インストール)
2. [Raspberry Shake の設定](#2-raspberry-shake-の設定)
3. [TUI ダッシュボード](#3-tui-ダッシュボード)
4. [シミュレーター](#4-シミュレーター)
5. [音声アラート](#5-音声アラート)
6. [P2P地震情報・EEW 表示](#6-p2p地震情報eew-表示)
7. [フィルタ検証](#7-フィルタ検証)
8. [アルゴリズム解説](#8-アルゴリズム解説)
...
LTA 窓を長くして誤検知を減らす：
```bash
.venv/bin/python3 jma_intensity_tui.py --station R38DC --lta 30 --trig 4.0
```
...
```
┌─ 監視中 ──────────────────────────────────────────┐
│ AM.R38DC  RS4D  100.0Hz  パケット:1234  稼働:00:05:00
│
│ ┌─ 計測震度 ────────────────────────────────────┐
│ │ 震度 3          I = 3.00   a = 5.402 gal     │
│ │ ████████████░░░░░░░░░░░░░░░░░░░  3.00 (3)    │
│ └───────────────────────────────────────────────┘
│
│ ┌─ 波形 (raw counts, DC除去) ───────────────────┐
...
実機なしで任意の震度を再現した UDP パケットを送出します。TUI の動作確認に使用します。
...
```bash
.venv/bin/python3 simulate_udp.py \
  --intensity 3.0 \
  --duration 60 \
  --quiet-sec 20 \
  --dest 127.0.0.1:9999
...
TUI 起動時に `http://localhost:50021` の VoiceVox Engine を自動検出します。起動していれば **No.7（アナウンス、速度 1.1 倍）** で読み上げます。
...
TUI 起動時に [P2P地震情報 WebSocket API](https://www.p2pquake.net/develop/json_api_v2/) へ自動接続し、以下の情報をリアルタイムで受信します。

| 情報 | 表示場所 | 内容 |
|------|----------|------|
| 地震情報 (code 551) | 最新地震情報パネル（下段右） | 直近5件の震源・M・最大震度・津波情報 |
| EEW（code 556） | EEW 行（STA/LTA パネルの下） | 震源・M・最大予測震度（参考値） |

### EEW について

- P2P地震情報経由の EEW は**内容・配信品質ともに無保証**です。表示は参考情報に留めてください。
- EEW が発報されていない間は「EEW なし」と表示されます。
- 取消された EEW は自動的に消去されます。
...
```bash
.venv/bin/python3 verify_filter.py
```
